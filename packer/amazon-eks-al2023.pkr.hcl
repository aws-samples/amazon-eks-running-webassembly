...
data "amazon-parameterstore" "this" {
  name   = "/aws/service/eks/optimized-ami/${var.eks_version}/${var.ami_type}/${var.architecture}/standard/recommended/image_id"
  region = var.region
}
...
build {
  ...
  provisioner "file" {
    source      = "templates/nodeadm-run.service"
    destination = "/home/ec2-user/nodeadm-run.service"
  }

  provisioner "file" {
    source      = "templates/containerd-config.toml"
    destination = "/home/ec2-user/containerd-config.toml"
  }

  provisioner "file" {
    source      = "templates/wasm-init.sh"
    destination = "/home/ec2-user/wasm-init.sh"
  }
